...
echo 'export AWS_REGION=$(curl http://169.254.169.254/latest/dynamic/instance-identity/document | jq -r .region)' >> /tmp/run.sh
echo 'LOCK_FILE=/tmp/run.lock' >> /tmp/run.sh
echo 'if test -f "$LOCK_FILE"; then' >> /tmp/run.sh
echo 'echo "$(date) Lock file present another process already running. Exiting."' >> /tmp/run.sh
echo 'exit 1' >> /tmp/run.sh
echo 'else' >> /tmp/run.sh
echo 'echo "$(date) Generating lock file $LOCK_FILE."' >> /tmp/run.sh
echo 'touch $LOCK_FILE' >> /tmp/run.sh
echo 'fi' >> /tmp/run.sh
echo 'TOOLS_PRESENT=$(aws s3 ls s3://#BUCKET#/tools/TPC-DSGen-software-code-3.2.0rc1.zip --region $AWS_REGION)' >> /tmp/run.sh
echo 'if test -z "$TOOLS_PRESENT"' >> /tmp/run.sh
echo 'then' >> /tmp/run.sh
echo 'echo "$(date) Tools not present on s3 bucket #BUCKET#. Retrying in 60 seconds"' >> /tmp/run.sh
echo 'rm -f $LOCK_FILE' >> /tmp/run.sh
echo 'exit 1' >> /tmp/run.sh
echo 'fi' >> /tmp/run.sh
echo 'aws s3 cp s3://#BUCKET#/tools/TPC-DSGen-software-code-3.2.0rc1.zip . --region $AWS_REGION' >> /tmp/run.sh
...
echo 'echo "$(date) Data generated. Shutting down ec2 instance now."' >> /tmp/run.sh
